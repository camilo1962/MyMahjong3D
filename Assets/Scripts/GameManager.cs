...
public class GameManager : MonoBehaviour
{
    ...
    [Tooltip("Número de segundos hasta que se restablece el multiplicador combinado.")]
    ...
    private void OnClickedOnFreeTile(object sender, Tile clickedTile)
    {
        ...
        if (clickedTile == lastSelectedTile) //Estamos haciendo clic en el mismo mosaico que la última vez, por lo que ignoramos esto
        {
            return;
        }
        if (clickedTile.tileType == lastSelectedTile.tileType) //tenemos un partido
        {
    ...
    //Methods of Testing
    [ContextMenu("Nivel de prueba borrado")]
    void TestLevelCleared() => OnNextLevelButtonPressed(null, null);
    [ContextMenu("Prueba GameOver Ganar")]
    void TestGameOverWin() => GameOver(true);
    [ContextMenu("Prueba GameOver Perder")]
